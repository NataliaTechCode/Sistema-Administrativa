...
function init(){
	...
	$.post("../ajax/departamento.php?op=5", function(r){
        console.log(r);
	    $("#departamento").html(r);
...
{
	$.post("../ajax/carrera.php?op=4",{idcarrera : idcarrera}, function(data, status)
	{
		...
		$("#nombre").val(data.nombrecarrera);
		 $.post("../ajax/departamento.php?op=5", function(r){
            $("#departamento").html(r);
            $('select[name=departamento]').val(data.iddepartamento);
            $('#departamento').trigger('change.select2');
        });
...
function desactivar(idcarrera)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/carrera.php?op=2", {idcarrera : idcarrera}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
...
function activar(idcarrera)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/carrera.php?op=3", {idcarrera : idcarrera}, function(e){
                console.log(e);
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
