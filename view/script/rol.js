...
function init(){
	...
	$.post("../ajax/permiso.php?op=2", function(r){
		console.log(r);
		$("#permiso").html(r);
		$('#permiso').trigger('change.select2');
	});
...
function limpiar()
{
	$("#nombre").val("");
	$("#idrol").val("");
	$('#permiso').val(null).trigger('change');
}
...
{
	$.post("../ajax/rol.php?op=4",{idrol : idrol}, function(data, status)
	{
		data = JSON.parse(data);		
		mostrarform(true);
		$("#nombre").val(data.rolnombre);
 		$("#idrol").val(data.idrol);
		$.post("../ajax/permiso.php?op=3&id="+data.idrol,function(r){
			$("#permiso").html(r);
			$('#permiso').trigger('change.select2');
		});
 	})
...
function desactivar(idrol)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/rol.php?op=2", {idrol : idrol}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
...
function activar(idrol)
{
	swal.fire({
		...
		if (result.value) {
			$.post("../ajax/rol.php?op=3", {idrol : idrol}, function(e){
				mensaje=e.split(":");
					if(mensaje[0]=="1"){  
						swal.fire(
							'Mensaje de Confirmación',
