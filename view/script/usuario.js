...
function init(){
	...
	$.post("../ajax/user.php?op=11", function(r){
	    $("#tipoestudiante").html(r);
		$('#tipoestudiante').trigger('change.select2');
	});

	$.post("../ajax/carrera.php?op=5", function(r){
	    $("#carrera").html(r);
...
function mostrar(idusuario)
{
	$.post("../ajax/user.php?op=4",{idusuario : idusuario}, function(data, status)
	{
		...
		$.post("../ajax/user.php?op=11", function(r){
			$("#tipoestudiante").html(r);
			$('select[name=tipoestudiante]').val(data.idtipoestudiante);
			$('#tipoestudiante').trigger('change.select2');
		});
	
		$.post("../ajax/carrera.php?op=5", function(r){
			$("#carrera").html(r);
			$('select[name=carrera]').val(data.idcarrera);
			$('#carrera').trigger('change.select2');
		});
